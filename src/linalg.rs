...
impl Vec2 {
    ...
    pub fn mag(&self) -> f64 { f64::sqrt(self.x * self.x + self.y * self.y) }
    pub fn norm(&self) -> Vec2 {
        let mag = self.mag();
        Vec2 { x: self.x / mag, y: self.y / mag }
    }

    pub fn almost_eq(self, rhs: Vec2) -> bool { (self - rhs).mag() < f64::epsilon() }
}

impl Zero for Vec2 {
    fn zero() -> Self { Vec2 { x: 0.0, y: 0.0 } }

    fn is_zero(&self) -> bool { self.almost_eq(Self::zero()) }
}

impl From<[f64; 2]> for Vec2 {
    fn from(value: [f64; 2]) -> Self { Vec2 { x: value[0], y: value[1] } }
}
impl From<[f32; 2]> for Vec2 {
    fn from(value: [f32; 2]) -> Self { Vec2 { x: value[0] as f64, y: value[1] as f64 } }
}

impl From<Vec2> for [f64; 2] {
    fn from(value: Vec2) -> Self { [value.x, value.y] }
}
impl From<Vec2> for [f32; 2] {
    fn from(value: Vec2) -> Self { [value.x as f32, value.y as f32] }
}
...
impl SubAssign<Vec2> for Vec2 {
    fn sub_assign(&mut self, rhs: Vec2) { self.x -= rhs.x; self.y -= rhs.y; }
}
...
#[derive(Copy, Clone)]
pub struct Mat3x3 {
    pub xx: f64, pub xy: f64, pub xw: f64,
    pub yx: f64, pub yy: f64, pub yw: f64,
    pub wx: f64, pub wy: f64, pub ww: f64,
}
...
impl Mat3x3 {
    pub fn translation(dx: f64, dy: f64) -> Mat3x3 {
        Mat3x3 {
            xx: 1.0, xy: 0.0, xw: dx,
            yx: 0.0, yy: 1.0, yw: dy,
            wx: 0.0, wy: 0.0, ww: 1.0,
        }
    }
    pub fn translation_vec2(vec2: Vec2) -> Mat3x3 { Self::translation(vec2.x, vec2.y) }
    pub fn rotation(radians: f64) -> Mat3x3 {
        Mat3x3 {
            xx: f64::cos(radians), xy: -f64::sin(radians), xw: 0.0,
            yx: f64::sin(radians), yy:  f64::cos(radians), yw: 0.0,
            wx: 0.0,               wy: 0.0,                ww: 1.0,
        }
    }
    pub fn det(&self) -> f64 {
    ...
    pub fn transpose(&self) -> Mat3x3 {
        Mat3x3 {
            xx: self.xx, xy: self.yx, xw: self.wx,
            yx: self.xy, yy: self.yy, yw: self.wy,
            wx: self.xw, wy: self.yw, ww: self.ww,
        }
    }
...
impl Add<Mat3x3> for Mat3x3 {
    ...
    fn add(self, rhs: Mat3x3) -> Self::Output {
        Mat3x3 {
            xx: rhs.xx * self.xx, xy: self.xy * self.xy, xw: self.xw * self.xw,
            yx: rhs.yx * self.yx, yy: self.yy * self.yy, yw: self.yw * self.yw,
            wx: rhs.wx * self.wx, wy: self.wy * self.wy, ww: self.ww * self.ww,
        }
    }
}
...
impl Mul<f64> for Mat3x3 {
    ...
    fn mul(self, rhs: f64) -> Self::Output {
        Mat3x3 {
            xx: rhs * self.xx, xy: rhs * self.xy, xw: rhs * self.xw,
            yx: rhs * self.yx, yy: rhs * self.yy, yw: rhs * self.yw,
            wx: rhs * self.wx, wy: rhs * self.wy, ww: rhs * self.ww,
        }
    }
}
impl Mul<Mat3x3> for f64 {
    ...
    fn mul(self, rhs: Mat3x3) -> Self::Output {
        Mat3x3 {
            xx: self * rhs.xx, xy: self * rhs.xy, xw: self * rhs.xw,
            yx: self * rhs.yx, yy: self * rhs.yy, yw: self * rhs.yw,
            wx: self * rhs.wx, wy: self * rhs.wy, ww: self * rhs.ww,
        }
    }
}
impl MulAssign<f64> for Mat3x3 {
    fn mul_assign(&mut self, rhs: f64) {
        self.xx *= rhs; self.xy *= rhs; self.xw *= rhs;
        self.yx *= rhs; self.yy *= rhs; self.yw *= rhs;
        self.wx *= rhs; self.wy *= rhs; self.ww *= rhs;
    }
}
...
impl Div<f64> for Mat3x3 {
    ...
    fn div(self, rhs: f64) -> Self::Output {
        Mat3x3 {
            xx: self.xx / rhs, xy: self.xy / rhs, xw: self.xw / rhs,
            yx: self.yx / rhs, yy: self.yy / rhs, yw: self.yw / rhs,
            wx: self.wx / rhs, wy: self.wy / rhs, ww: self.ww / rhs,
        }
    }
}
...
impl MulAssign<Mat3x3> for Vec2 {
    fn mul_assign(&mut self, rhs: Mat3x3) {
        (self.x, self.y) = (rhs.xx * self.x + rhs.xy * self.y + rhs.xw * 1.0,
